...
// Check the color of a byte.
int isColorBlack(byte a){
    if (a == BLACK_Pixel){
        return 1;
    }
    else{
...
public:
    ...
    int step;
    int side;
    frame(int x, int y, int step2, int side2){coordx=x; coordy=y; step=step2; side=side2;};
    // 0 : all white, 1 all black, 2 both
    int tellColor(byte* image){
        int temp = 0;
        if (isColorBlack(image[coordx*side + coordy])==1){
            temp = 1;
        }
        else{temp = 0;}
        for(int i = coordx; i<(coordx+step); i++){
            for (int j = coordy; j<(coordy+step); j++){
                if (isColorBlack(image[i*side+j]) != temp){
                    return 2;
                }
            }
        }
    ...
    int tellGreyColor(byte* image){
        int temp = int(image[coordx*side + coordy]);
        for(int i = coordx; i<(coordx+step); i++){
            for (int j = coordy; j<(coordy+step); j++){
                if (int(image[i*side+j]) != temp){
                    return 256;
                }
            }
        }
...
//    
    ...
    void fillFrameWith(byte* image, byte col){
        for(int i = coordx; i<(coordx+step); i++){
            for (int j = coordy; j<(coordy+step); j++){
                image[i*side+j]=col;
            }
